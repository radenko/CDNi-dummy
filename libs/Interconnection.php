...
class Interconnection {
    ...
    function peerSetOfferLocalStatus($interconID,$status) {     
        ...
        if ($this->db->errno()) echo $this->db->error ();
            ...
            $this->clients[$peerURL]->setOfferLocalStatus($this -> config['id'],  $status); //Fout?? hij gaat nu de status van "cdn.ngnlab.eu" proberen te updaten terwijl die natuurlijk niet in de interconnections database zit...
			$updatelocalstatusresult = array("a" => $peerURL,"b" => $this -> config['id'],"c" => $status);
			return $updatelocalstatusresult;
        }        
    }
    ...
    function processLocalOffers() {  
        ...
        if (isset($this -> config['i']['peers'])) { //Not set, since not in config.php
		...
		//echo "Result of processLocalOffers selection:"; var_dump($result); echo "\n";
		
        if ($this -> db -> errno()) echo $this->db->error ();
        else {
            while ( $peer = $this -> db -> fetch_assoc() ) { //Does not enter the while loop, localStatus seems NULL?!?!
    ...
    function processContentForTransfer() {
		...
		echo "Entering processContentForTransfer \n";
		
        $this -> addAllPeers();
        $intercons=$this->getAllInterconnections();
        
        $contenttest = $this -> db -> select('content');
		$contenttest2 = mysql_fetch_assoc($contenttest);
		echo "Results van select content: \n"; var_dump($contenttest2);
    ...
    function processLocalContentForTransfer() {
		...
		echo "Entering processLocalContentForTransfer \n";
        ...
        if ($this-> db ->errno()) echo $this -> db->error () . PHP_EOL;
            ...
            if (!is_numeric($interconID)) echo "interconID is not numeric".PHP_EOL;
                ...
                if ($this->db->errno()) echo $this->db->error ().PHP_EOL;
                    ...
                    //return true; 
					$caps = mysql_fetch_row($this->db->query("SELECT * FROM peerCapabilities;"));
					$offer = mysql_fetch_row($this->db->query("SELECT * FROM interconnections;"));
					return array("peerCapabilities" => $caps,"Offerstatus" => $offer);
                }
            }
        }
...
//            echo "found interconnection: $interconID \n";
            if (!is_numeric($interconID)) echo "interconID is not numeric".PHP_EOL;
                ...
                if ($this -> db -> errno()) echo $this -> db -> error () . PHP_EOL;
                    ...
                    $updatelocalstatusresult = $this -> updateCompleteStatus($interconID);
            
                    //return true;
					$caps = mysql_fetch_row($this->db->query("SELECT * FROM peerFootprints;"));
					$offer = mysql_fetch_row($this->db->query("SELECT * FROM interconnections;"));
					return array("localStatusUpdate" => $updatelocalstatusresult,"peerCapabilities" => $caps,"Offerstatus" => $offer);
                }
            }
        }
    ...
    function setOffer($CDNid,$peerURL) {
        ...
        $insertresult = $this->db->insertUpdate("interconnections",
                array(
                    'CDNid' => $CDNid,
                    'peerURL' => $peerURL,
                    'peerStatus' => 'offer'
        ...
        if ($this->db->errno()) echo $this->db->error() . PHP_EOL;
            ...
            return array("CDNid" => $this -> config['id'],"Inserted:" => $insertresult);
        }
        ...
        elseif ($this->db->num_rows($qr) && ($cnt=$this->db->result()) >0 ) {
            ...
            elseif (  $this->db->num_rows($qr) && ($cnt=$this->db->result()) > 0  ) {
                ...
                if ($this->db->errno()) echo $this->db->error ().PHP_EOL;
                    ...
                    $updatelocalstatusresult = $this->peerSetOfferLocalStatus($interconID, 'complete');
                    //return true;
					return $updatelocalstatusresult;
                }
            }
