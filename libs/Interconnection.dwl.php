...
class Interconnection {
    ...
    function addPeer($params,$replace=false) {
        ...
        $this -> addPeer2($id,$url,$replace);
    }
    
    function addPeer2 ($id,$url,$replace=false) {
    ...
    function peerSetOfferLocalStatus($interconID,$status) {     
        $this->db->query("SELECT peerURL FROM interconnections WHERE interconID='". $this->db->escape_string($interconID) ."';");       
        if ($this->db->errno()) echo $this->db->error ();
        else {
            $peerURL = $this->db->result();
    ...
    function processLocalOffers() {        
        if (isset($this -> config['i']['peers'])) {
            foreach ($this -> config['i']['peers'] as $peer) {
                ...
                $this -> db -> select('interconnections',array('COUNT(*)'),array('WHERE' => "peerURL='". $this->db->escape_string($peer['APIurl']). "'"));
